...
Let’s start by installing piece of code set up the directory of the kaggle notebook.
...
A PyTorch implementation of Gemma 7B instruction tuned model. It is a 7B `int8`
quantized parameter base model that has been instruction-tuned so it can respond
to prompts in a conversation manner.
...
## Submission
https://linuxhandbook.com/pigz/
```{bash}
!apt install pigz pv > /dev/null
```

Using pigz, creating a `submission.tar.gz` inside the directory `kaggle/working/submission`
```{bash}
